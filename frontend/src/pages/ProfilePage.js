...
export default function ProfilePage() {
  ...
  const checkinMutation = useMutation(userAPI.checkin, {
    onSuccess: () => toast.success('✅ Günlük check-in kaydedildi'),
    onError: () => toast.error('Check-in kaydedilemedi'),
  });

  const sleepMutation = useMutation(userAPI.logSleep, {
    onSuccess: () => toast.success('😴 Uyku verisi kaydedildi'),
    onError: () => toast.error('Uyku verisi kaydedilemedi'),
  });

  const BRAIN_TYPES = { analytical: { icon: '🔢', label: 'Analitik', color: '#00d4ff' }, creative: { icon: '🎨', label: 'Yaratıcı', color: '#7c3aed' }, empathetic: { icon: '💙', label: 'Empatik', color: '#10b981' }, strategic: { icon: '♟️', label: 'Stratejik', color: '#f59e0b' } };
  const bt = BRAIN_TYPES[user?.neuroProfile?.brainType];

  return (
    <MainLayout>
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        <h1 style={{ fontSize: 32, fontWeight: 800, marginBottom: 32 }}>👤 Profilim</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
          {/* Edit profile */}
          <div className="card">
            <h3 style={{ fontWeight: 700, marginBottom: 20 }}>✏️ Profili Düzenle</h3>
            <form onSubmit={handleSubmit((data) => updateMutation.mutate(data))}>
              ...
              <button type="submit" className="btn btn-primary" disabled={updateMutation.isLoading}>
                {updateMutation.isLoading ? <div className="loading-spinner" /> : 'Kaydet'}
              </button>
            </form>
          </div>
          ...
          {user?.neuroProfile?.brainType && (
            <div className="card">
              <h3 style={{ fontWeight: 700, marginBottom: 20 }}>🧠 Nörolojik Profil</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[
                  { label: 'Beyin Tipi', value: bt ? `${bt.icon} ${bt.label}` : '—' },
                  { label: 'Enerji Ritmi', value: { morning: '🌅 Sabahçı', evening: '🌙 Akşamcı', flexible: '⚡ Esnek' }[user.neuroProfile.energyRhythm] || '—' },
                  { label: 'Karar Stili', value: { rational: '🎯 Rasyonel', intuitive: '✨ Sezgisel', balanced: '⚖️ Dengeli' }[user.neuroProfile.decisionStyle] || '—' },
                  { label: 'Sosyal Örüntü', value: { introvert: '🔇 İntrovert', extrovert: '🎉 Extrovert', ambivert: '🔄 Ambivert' }[user.neuroProfile.socialPattern] || '—' },
                  { label: 'Nöro Skor', value: `${user.neuroProfile.overallScore || 0}/100` },
                ].map((item) => (
        ...
        <div className="card" style={{ marginBottom: 24 }}>
          <h3 style={{ fontWeight: 700, marginBottom: 20 }}>📊 Günlük Check-in</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20, marginBottom: 20 }}>
            {[
              { key: 'mood', label: '😊 Ruh Hali', color: '#00d4ff' },
              { key: 'energy', label: '⚡ Enerji', color: '#10b981' },
              { key: 'stress', label: '😤 Stres', color: '#ef4444' },
              { key: 'focus', label: '🎯 Odak', color: '#7c3aed' },
            ].map((item) => (
          ...
          <button onClick={() => checkinMutation.mutate(checkinData)} className="btn btn-primary" disabled={checkinMutation.isLoading}>
            {checkinMutation.isLoading ? <div className="loading-spinner" /> : '✅ Check-in Kaydet'}
          </button>
        </div>

        {/* Sleep tracker */}
        <div className="card">
          <h3 style={{ fontWeight: 700, marginBottom: 20 }}>😴 Uyku Takibi</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16, marginBottom: 20 }}>
            <div>
              <label className="form-label">Yatış Saati</label>
          ...
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 14 }}>
              <span style={{ color: '#94a3b8' }}>Uyku Kalitesi: {sleepData.quality}/10</span>
              <span style={{ color: '#7c3aed' }}>{sleepData.duration} saat</span>
            </div>
          ...
          <button onClick={() => sleepMutation.mutate(sleepData)} className="btn btn-secondary" disabled={sleepMutation.isLoading}>
            {sleepMutation.isLoading ? <div className="loading-spinner" /> : '😴 Uyku Kaydını Ekle'}
          </button>
        </div>
      </div>
    </MainLayout>
